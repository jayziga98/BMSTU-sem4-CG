...
void MainWindow::fixVertices(QVector<QPoint>& intersections)
{
    ...
    for (int k = 0; k < intersections.size(); k++) {
        ...
        if (ui->delayCheckBox->isChecked()) {
            QTest::qWait(ui->delaySpinBox->value());
        }
    }
    displayImage();
}
...
void MainWindow::on_fillPushButton_clicked()
{
    ...
    if (!ui->delayCheckBox->isChecked())
        QMessageBox::information(this, "Время выполнения", QString::number(ms_double.count()).append("миллисекунд"));
}
...
void MainWindow::resizeEvent(QResizeEvent* event)
{
   ...
   pixmap = pixmap.scaled(ui->drawLabel->width(), ui->drawLabel->height(), Qt::IgnoreAspectRatio);
   image = image.scaled(ui->drawLabel->width(), ui->drawLabel->height(), Qt::IgnoreAspectRatio);
   ui->drawLabel->setPixmapPointer(pixmap);
}
